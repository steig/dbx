...
# Sample N rows from a Postgres JSON/JSONB column and emit unique
# JSON-path strings (jq-syntax: `$.contact.email`). Best-effort path
# discovery — anything the sample misses won't appear. Used by
# `dbx scrub init` to suggest jsonb_scrub_paths entries.
# Args: $1 host, $2 database, $3 table, $4 column, $5 sample_size
scrub_schema_sample_pg_json() {
  local host="$1" db="$2" table="$3" col="$4" n="${5:-100}"

  # Validate identifiers to keep them out of injection territory.
  if ! [[ "$table" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || \
     ! [[ "$col"   =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then
    log_error "scrub_schema_sample_pg_json: invalid identifier: '$table'.'$col'"
    return 1
  fi
  ...
  # jsonb_object_keys would only give us top-level keys. Use a
  # recursive CTE-like jq-style walk via jsonb_path_query on the
  # client side: for each sampled row, walk the JSON and emit every
  # path-to-leaf. Done in psql via a function-free expression to
  # keep this read-only on the source DB.
  #
  # Postgres has `jsonb_each_text` but no recursive path walker out
  # of the box. We pull the raw JSON rows back and walk them in jq
  # locally — cheaper than installing a function on the source.
  local sql="SELECT \"$col\"::text FROM \"$table\" WHERE \"$col\" IS NOT NULL LIMIT $n;"
  local rows
  rows=$(docker exec -i -e PGPASSWORD="$db_pass" "$POSTGRES_CONTAINER" \
    psql -h "$db_host" -p "$db_port" -U "$db_user" -d "$db" \
    -tA -c "$sql" 2>/dev/null) || return 0

  [[ -z "$rows" ]] && return 0

  # Walk every sampled row and emit unique `$.a.b.c` paths. We treat
  # arrays as opaque (use the key without an index) — matters because
  # path-to-leaf for an array of primitives would otherwise blow up
  # with per-element entries.
  local row
  while IFS= read -r row; do
    ...
    jq -r '
      def walk_paths(prefix):
        if type == "object" then
          to_entries[] | (prefix + "." + .key) as $p | (.value | walk_paths($p))
        elif type == "array" then
          (.[0] // null) | walk_paths(prefix)
        else
          prefix
        end;
      walk_paths("$")
    ' <<<"$row" 2>/dev/null || true
  done <<<"$rows" | sort -u | grep -v '^\$$' || true
}
