...
- **`.meta.json` inputs reach jq via `--slurpfile`, so a wide schema cannot E2BIG the exec (#242).** The `source_extensions` and (especially) `scrub_schema` blobs scale with the database, and a wide-enough schema pushed `--argjson` past the kernel's per-argument limit. The E2BIG exec failure wrote 0-byte meta files, which later forced `--skip-verify` on restore. Anything schema-sized now goes to jq through temp files, never argv.

## [0.40.1] - 2026-07-26

### Fixed

- **v0.40.0 could not be installed at all; this release is the fix (#233).** Its `SHASUMS256.txt` recorded `76e8ebfd…` for `dbx` and `aa4d8fdc…` for `man/man1/dbx.1`, while the tag actually carried `4937c20d…` and `15588f15…`. `install.sh` did exactly what it is built to do — refused the download and left the existing install untouched — so every `curl | bash` and every `dbx update` against v0.40.0 stopped on `Checksum mismatch for dbx`. That is a total outage of the upgrade path, not a cosmetic drift. The manifest is regenerated here and verified against all 48 shipped files. **Homebrew and Nix installs were never affected:** both pin the source tarball's own digest and never read this manifest. If you are on v0.40.0, `curl | bash` from `main` works again, and `dbx update` picks this release up.

### Added

- **Unit coverage for `dbx update` (#233).** The `--force` flag and the already-on-latest no-op shipped in 0.40.0 with their tests written but never committed. `tests/unit/update_cmd.bats` stubs the release resolvers and `curl`, so it asserts the install *decision* — no-op when current or ahead, install when a newer tag exists, `--force` reinstalling the current version, the fall back to `main` when no tag resolves, argument rejection, and `--help` — without touching the network.

- **A tracked `pre-push` hook runs the release drift guard as the last thing before bytes leave the machine.** v0.40.0 shipped a `SHASUMS256.txt` that contradicted its own payload — `76e8ebfd…` recorded for `dbx` against the `4937c20d…` actually tagged, and `aa4d8fdc…` for `man/man1/dbx.1` against `15588f15…` — so `install.sh` did what it is built to do, refused the download, and every `curl | bash` and `dbx update` against that release died on `Checksum mismatch for dbx`. **The gap was ordering, not a missing check.** `scripts/release.sh` regenerates the manifest from what actually landed and then runs `scripts/check-release-consistency.sh` as its final gate; it did both, correctly. The `dbx update --force` work was then folded into the same `chore: release 0.40.0` commit, editing `dbx` and `man/man1/dbx.1` *after* that gate had passed, and the commit went straight to `main`, so CI's `release-consistency` job never ran against it either. A gate that runs before the last edit is not a gate. `.githooks/pre-push` closes that window: it gates pushes to `main` and to `v*` tags — the two refs a release travels on — and refuses the push when the payload disagrees with the manifest. Other branches are not gated, so ordinary work is untouched, and `git push --no-verify` still overrides it. It is installed by `scripts/install-hooks.sh`, which points `core.hooksPath` at the tracked `.githooks/` directory, because `.git/hooks` does not travel with a clone and a guard every contributor must reinstall by hand is the same class of problem it exists to prevent; the directory is `.githooks/` rather than the conventional `hooks/` because `.gitignore` reserves a top-level `/hooks/` for restore-prep operator artifacts that must never reach this public repo. The guard inspects the working tree rather than the commit range being pushed, so a push made from a dirty tree can fail on uncommitted changes — the conservative direction, and `release.sh` already refuses to run against one. Verified against v0.40.0's actual manifest: the hook refuses the tag push and lets feature branches through.

## [0.40.0] - 2026-07-26

### Added

- **A Homebrew tap (#137).** `curl | bash` was the only install channel, which is a trust speed-bump for exactly the ops audience dbx targets, and it left uninstall and dependency handling as manual prose in the docs. `brew tap steig/dbx https://github.com/steig/dbx && brew install dbx` now installs the launcher, all 14 libraries, the wizard assets, all 19 man pages and bash/zsh/fish completions, with `jq` and `zstd` pulled in automatically and `brew uninstall dbx` removing the lot. The second argument to `brew tap` is required and not a typo: the formula lives in this repository as `Formula/dbx.rb` rather than in a separate `steig/homebrew-dbx`, so the drift guard and CI can see it — a second repository would be a second hand-maintained copy of the version, which is the failure mode #122 already cost us. Docker is deliberately *not* a formula dependency: Homebrew's is the Docker Desktop cask, while Colima, OrbStack, Podman and Rancher Desktop all serve dbx equally well, so the caveats name the requirement instead of choosing a 700 MB GUI on the user's behalf. `dbx` and `lib/` are installed together under `libexec` with a `bin/dbx` wrapper that execs the launcher, because dbx resolves its libraries as `$(dirname "$0")/lib` and a symlink would send it looking in `bin/lib`; the wrapper means the formula patches not one shipped byte, so Homebrew's tarball digest genuinely covers what lands on disk. `dbx update` still works but should not be used on a Homebrew install — it re-runs the curl installer and leaves a second, unmanaged copy in `~/.local/bin`; `brew upgrade dbx` is the right command, and the caveats say so. See [docs/homebrew.md](docs/homebrew.md).
- **`scripts/update-formula.sh` repoints the formula, with a digest that is computed rather than typed (#137).** A formula pins the sha256 of its own tag's source tarball, and that tarball contains the formula — so no fixed point exists and the release commit cannot bump it. It is repointed in a second commit after the tag is pushed, which is fine because Homebrew reads formulae from `main`, never from a tag. The correctness of that arrangement is split across two checks by what each can see: `scripts/check-release-consistency.sh` gains a sixth check that runs offline on every PR, asserting the pinned tag has a `CHANGELOG.md` release heading (so it names a release that was actually cut, not one somebody invented) and that the digest is 64 hex characters (so a placeholder cannot ship) — deliberately *not* an `== VERSION` check, which would fail on every release commit; and `scripts/update-formula.sh --check` re-downloads the pinned tarball and compares digests, which is the half that needs the network and the one that catches the failure users feel, since a wrong digest makes every `brew install` fail. A new `.github/workflows/formula.yml` runs the digest check whenever the formula changes and once a week besides, and runs `brew install --build-from-source` plus `brew test` on a macOS runner. The formula's `test do` block asserts that the installed `dbx version` matches the version the formula claims, that `dbx help` works, and that `dbx config init` writes a config — no Docker, no network. The release procedure in [CONTRIBUTING.md](CONTRIBUTING.md#cutting-a-release-maintainers) carries the extra step.
- **A Nix flake: `packages.default`, `apps.default`, `devShells.default` and `nix flake check` (#138).** `nix run github:steig/dbx -- backup prod` and `nix profile install github:steig/dbx` now work, alongside `install.sh` and Homebrew. The package installs the launcher and `lib/` as siblings under `libexec/dbx/` and exposes only a `makeWrapper` wrapper in `bin/`, because `dbx` resolves `LIB_DIR` as `$(dirname "$BASH_SOURCE")/lib` — copying the launcher on its own into `bin/` breaks every `source` on line 36. The 19 man pages and the bash/zsh/fish completions (generated by running the freshly built `dbx completion` at build time) go in as well. `jq`, `zstd`, `curl`, `ssh`, `gpg`, `age`, `openssl`, `gum`, `fzf`, `pv` and `python3` are wrapped onto `PATH`, plus `secret-tool` and `xdg-open` on Linux — `secret-tool` in particular because `vault_backend` prefers it over `gpg-file`, and putting `gpg` on `PATH` without it would silently demote a Linux user from their keyring. **Docker is deliberately not wrapped:** the daemon cannot be vendored and a pinned client would fight whatever Docker Desktop, colima or podman already installs. Neither is an S3 client — `dbx storage` is opt-in and `mc` and `aws` are interchangeable there. The package version is read out of the `VERSION=` line in `dbx` at evaluation time rather than restated, so `flake.nix` is not a 22nd file for `scripts/release.sh` to bump or `scripts/check-release-consistency.sh` to police (#122). The build asserts `dbx version` prints that version, which fails the build if `LIB_DIR` resolution ever breaks. `dbx update` re-runs `install.sh` into `~/.local` and has no business touching a store path, so the wrapper defaults `DBX_NO_UPDATE_CHECK=1`; set it to `0` to get the notice back. Systems are `aarch64-darwin`, `aarch64-linux` and `x86_64-linux` — nixpkgs 26.11 has dropped `x86_64-darwin`, and listing it would make `nix flake show --all-systems` throw instead of skip, so Intel macs stay on `install.sh` or Homebrew.
- **`nix develop` replaces assembling the toolchain by hand (#138).** The devShell carries everything `.github/workflows/ci.yml` runs — `bats`, `parallel` (for `bats -j`), `shellcheck`, `ruff`, `actionlint`, `just`, `jq`, `zstd`, `age`, `gnupg`, `mc`, `curl`, `python3` — at one pinned set of versions, so contributors stop reaching for `nix shell nixpkgs#bats --command …` on every invocation and stop disagreeing about which `shellcheck` they ran. `flake.lock` is committed for exactly that reason. Docker is not in the shell: `bats tests/integration/` uses the host daemon. `nix flake check` runs the lint gate, `bash -n`, `scripts/check-release-consistency.sh` and the package build. **Neither test suite is a check**, both for the same reason — a check that can never pass is worse than no check. The integration suite needs Docker, which a build sandbox has no access to. The unit suite writes its `docker`/`gpg`/`mc` stubs at runtime from `#!/usr/bin/env bash` heredocs, and a sandbox has no `/usr/bin/env`: 117 of 904 tests fail on that alone, and `patchShebangs` cannot fix stubs that do not exist until the tests run. Both suites are green inside `nix develop`. Documented in [`docs/nix.md`](docs/nix.md).

### Changed

- **Host aliases, safety levels and `dbx host add` moved to `lib/host.sh` (#145).** The dispatcher owned a subsystem instead of routing to one: `host_add` and the config-block writers it drives (`host_write_block`, `host_delete_block`, `host_prompt_port`) plus `cmd_host` sat inline in `dbx`, while the alias and safety helpers they depend on (`host_alias_valid`, `host_exists`, `host_safety`, `require_writable_host`, `require_writable_host_for_restore_into`) sat in `lib/core.sh` between the vault and audit code, so the two halves of one subsystem were 2,500 lines and a file apart. All ten now live in `lib/host.sh`, sourced immediately after `core.sh` so the safety gates are defined before `lib/post_restore.sh`, `lib/scrub.sh` and the restore path read them. A pure move: not one line of logic changed, and the extraction is visible as a whole-block delete plus a new file rather than a rewrite. `install.sh`'s download list, `SHASUMS256.txt` and the test helper's source order carry the new file; `scripts/check-release-consistency.sh` fails the build if any of the three is missed. No user-visible change.

- **`cmd_restore` decomposed into named steps (#145).** At 646 lines it was the largest function in the repo and the one least amenable to review: argument parsing, both `safety=prod` gates, the remote download, source resolution, checksum verification, target-name derivation, database-type detection, engine dispatch, post-restore hooks and the PII scrub gate ran as one unbroken block sharing some twenty locals, so a reader had no way to bound what any one line could touch. It is now a sixty-line sequence of `_restore_parse_args`, `_restore_early_into_gate`, `_restore_hooks_only`, `_restore_fetch_remote`, `_restore_resolve_source`, `_restore_verify_checksum`, `_restore_target_name`, `_restore_detect_db_type`, `_restore_scrub_preflight`, `_restore_dispatch`, `_restore_run_hooks`, `_restore_scrub_gate` and `_restore_cleanup_download`, each carrying a header naming the caller locals it reads and the ones it writes. The steps share `cmd_restore`'s locals by name rather than taking arguments and echoing results, which is deliberate and not laziness: `die` has to terminate dbx, and a step that ran inside a command substitution would kill only the subshell — a restore that was supposed to abort would carry on with an empty variable. For the same reason every step is called unconditionally, so `set -e` applies inside it exactly as it did inline (calling one in an `if` or after `||` would disable errexit through its whole body), and the single step that has to stop the restore early — the operator escaping out of the `fzf` picker — sets a flag rather than returning non-zero. Behaviour-preserving, and checked as such rather than asserted: the new bodies were assembled from the original line ranges and diffed against them, leaving the function boundaries and that one flag as the only added lines. Unit coverage for the flag guards, the derived target name and the database-type fallback chain was written first and passes unchanged either side of the move.
- **`lib/wizard-server.py` dispatches through a route table instead of two if/elif chains (#146).** `do_GET` and `do_POST` walked ~30 `if path == …` arms across 290 and 600 lines, and every POST arm re-implemented the same body preamble: parse `Content-Length`, bounds-check it, `rfile.read`, `json.loads`, handle the decode error, check the result is an object — twelve copies of six lines, which is twelve places for a cap to be forgotten. Each verb now runs its gates and then looks the path up in a `{path: handler}` dict, with a separate tuple of compiled patterns for the only parameterised routes there are (`/api/jobs/<32-hex-id>/events` and `…/cancel`); each endpoint is its own method. The body preamble collapses into one `read_json_body(handler, max_len)` whose size cap is a required argument rather than a default, so a new endpoint cannot quietly ship without one — an unbounded `Content-Length` is a single-request DoS on a server `dbx serve` may expose beyond loopback. Every per-route cap, status code and error string is preserved, including the endpoints that answer a non-object body as JSON rather than as text and `/api/schedules`' distinct message. **The security ordering is unchanged and load-bearing:** the `Host` allowlist (#126) still runs first and the token check (#123/#124) second, both before any route lookup, because under `--no-auth` the Host check is the only per-request gate — so an unknown path is refused by the gates before it can 404, exactly as before. Behaviour-preserving; no user-visible change.
...
- **`mc` aliases are now `dbx_storage` / `dbx_storage_<backend>`, not `dbx-storage…` (#217).** The alias is the suffix of the `MC_HOST_<alias>` variable carrying the credentials, and a hyphen cannot appear in a shell variable name; backend names admitting `.` and `-` are mapped to `_` for the same reason. The alias is internal — it appears in dbx's own "Uploading to: …" lines and nowhere in configuration — so nothing needs changing.
- **Losing `mc alias set` costs no diagnosis (#212 preserved).** That call happened to be the first thing to touch the network, so it was where bad credentials and an unreachable endpoint surfaced. Both now surface from the first real transfer, which has carried `mc`'s own stderr since 0.38.0, and remain distinguishable: `The request signature we calculated does not match` versus `dial tcp …: connect: connection refused`. No connectivity probe was added in its place — the obvious one, `mc ls <alias>`, needs `ListBuckets`, which scoped keys (R2 API tokens in particular) frequently lack, and would reject working configurations. The one thing the removal did lose is caught explicitly instead: an `s3.endpoint` with no `http://` or `https://` is now named as such, where `mc` answers only `Invalid arguments provided`.

## [0.39.0] - 2026-07-25

### Security

- **`install.sh` verifies every file it downloads against `SHASUMS256.txt` (#140).** The `curl | bash` installer fetched the launcher, 27 library files and 19 man pages with no integrity check of any kind, straight into their final locations — so a truncated body, a corrupted proxy or a CDN serving a half-updated ref installed silently, and the man-page loop's `|| true` swallowed per-file failures on top of that. Each release now ships a generated `SHASUMS256.txt`; the installer fetches it for the same ref, downloads everything into a staging directory, checks every file, and only then moves anything into place. A mismatch aborts with the expected and actual digest and leaves the existing install byte-for-byte untouched, and a man page the manifest lists but the download missed is now an error instead of a silent skip. **What this does not do is establish provenance:** the manifest comes from the same origin as the files it describes, and `install.sh` is itself fetched unverified, so anyone able to tamper with the origin can serve a matching manifest. It defends against corruption, truncation and cache skew — not against a compromised origin. That needs a signature checked against an out-of-band key, which is not implemented. Releases up to 0.38.0 predate the manifest, so installing one of those tags warns and continues; set `DBX_REQUIRE_CHECKSUMS=1` to refuse anything unverifiable instead. The manifest is generated by `scripts/gen-shasums.sh` from the same file list the release drift guard uses, `scripts/release.sh` regenerates it as part of the version bump, and CI fails any PR that changes a shipped file without regenerating it.
- **The container image's `mc` download is pinned and checksummed (#140).** `docker/Dockerfile` fetched the MinIO client from the floating `…/release/linux-${TARGETARCH}/mc` path with no version, no checksum and no retry, so the published image contained whatever binary min.io served at build time and an unlucky build shipped it unverified. It is now pinned to `RELEASE.2025-08-13T08-35-41Z`, fetched from the immutable archive path with `--retry 5`, and checked against the per-architecture digest min.io publishes beside it — a failed check fails the build.
...
- **Audit log is created at `600`, never briefly world-readable (#150).** `audit_log` appended the entry first and ran `chmod 600` after, so the very first write created `~/.local/share/dbx/audit.log` at the ambient umask mode (typically `644`). In that window any other local user could open the log and keep the descriptor, which survives the chmod and then streams every subsequent entry — host names, database names, operations and outcomes. The log is now created empty under `umask 077` before the entry is built, and the chmod runs on the empty file, so it never holds content at a permissive mode. Logs created by older versions are still repaired to `600` on the next write; format and content are unchanged.
- **`age` recipients are derived from the whole identity file (#150).** `init_age_encryption` extracted the public key with `grep -v '^#' identity | head -1 | age-keygen -y`, taking whatever sat on the first non-comment line rather than asking age about the file. On a multi-key identity file that silently published only the first key's recipient, so new backups could not be decrypted by the other identities; on a file with a leading blank line it fed `age-keygen` an empty line, produced no key, and fell through to generating a *new* key pair over the existing identity. It now calls `age-keygen -y "$identity_file"` directly, writing every recipient the file yields. Single-key identity files behave exactly as before.

### Changed

- **Exit-time cleanup is registered, not trapped (#149).** Bash has one trap slot per signal, so a second `trap … EXIT` silently replaces the first and the earlier cleanup never runs. Three sites installed one: `setup_security_trap` (the credential scrub, installed at startup), `create_ssh_tunnel` (tunnel teardown, which re-chained the scrub by hand), and `cmd_wizard` (kills the wizard server, removes the done-marker and log) — which did *not* re-chain, so a `dbx wizard` run never ran the scrub. Nothing leaked there, because the wizard holds no DB credentials; the point is that keeping it correct depended on every future author noticing, and the next site to forget would drop a teardown that does matter. All three now call `register_exit_handler '<command>'` (`lib/core.sh`), which appends to a list run by a single dispatcher, and a test fails the build if a raw `trap … EXIT` reappears outside the registry. Handlers run LIFO, so teardown happens in reverse order of setup and the scrub — registered first — runs last, while the credentials later handlers may need are still set. Each handler runs with `errexit` off and its failure swallowed, so one broken handler cannot skip the rest, and the status the script was exiting with is returned untouched. The `RETURN` traps that clean up per-function temp files are left alone: they are function-scoped, one per function, and cannot clobber each other.

- **`INT`/`TERM` now re-raise instead of returning (#149).** The old handlers ran cleanup and returned, leaving the shell to resume whatever it had been interrupted in, so the exit status came from wherever the signal happened to land. Cancelling `dbx wizard` measured `1` both for a Ctrl-C and for a `kill -INT` at the dbx process alone — and the second case additionally fell through to the wait loop, found the server the handler had just killed, and reported `Wizard server exited unexpectedly`. The signal is now re-raised against the shell after cleanup, so dbx dies of the signal itself: Ctrl-C and `kill -INT` both give `130`, `kill -TERM` gives `143`. Cleanup also no longer runs twice on the signal-then-exit path.

- **Integration tests now also run on macOS (#142).** `integration-tests` was ubuntu-only, so the round-trip paths — where BSD-vs-GNU `sed`/`date`/`stat`/`wc` differences hide — had no macOS signal at all, on a tool that targets macOS heavily. A new `integration-tests-macos` job runs the same suite on `macos-latest`; the Docker-dependent files skip themselves via `require_docker`, leaving the host-side bash under test. Full Docker coverage is not possible there: dbx runs `pg_dump`/`psql`/`mysqldump` *inside* the managed containers, and GitHub's macOS runners cannot host a daemon — the arm64 images have no nested virtualization, so Colima/Lima cannot boot, and `macos-13` (the last Intel image where that worked) was retired in December 2025. Because bats scores an all-skipped run as a pass, the job also runs `scripts/check-integration-coverage.sh`, which fails unless a floor of tests actually executed — a job that only *looks* like it tests macOS is worse than none. The split immediately caught two real defects: BSD `wc -l` padding its count with leading spaces, which broke six `clean` assertions, and the `stat` dialect bug listed under Fixed.

- **Releases are cut by `scripts/release.sh` instead of by hand (#147).** A release bumps 21 files that all have to agree — `VERSION` in `dbx`, the `.TH` version *and* date of all 19 man pages, and the `CHANGELOG.md` `[Unreleased]` roll — which is how `dbx-build-image.1` once shipped stale (#122). `scripts/release.sh [--dry-run] <X.Y.Z | major | minor | patch>` does the whole pass, refuses a dirty tree / a non-newer version / an empty `[Unreleased]`, and finishes by running `scripts/check-release-consistency.sh` — the drift guard CI already runs on every PR. It takes its file list from that guard rather than keeping a second copy, so the bump and the check cannot disagree. Tagging, pushing, and `gh release create` stay manual; the script prints them. See [Cutting a release](CONTRIBUTING.md#cutting-a-release-maintainers).

### Fixed

- **The published `linux/arm64` image no longer carries an amd64 `mc` (#140).** `docker/Dockerfile` declared `ARG TARGETARCH=amd64`. That default does not mean "amd64 unless building for something else" — it *shadows* the value BuildKit injects, so `TARGETARCH` was `amd64` on every platform and the arm64 image downloaded the amd64 MinIO client, which cannot execute there. Every `dbx storage` operation in the arm64 image failed at `mc`. The ARG is now declared bare, so BuildKit's value comes through; a build that supplies nothing (the legacy non-BuildKit builder) fails loudly rather than silently picking the wrong architecture. Verified by building both platforms and checking `mc --version`.

- **`dbx storage add` and `dbx host add` no longer crash on `require_gum` (#209).** Both wizards call `require_gum` as their dependency check, but the function was never written — it existed only in the design doc — so instead of a dependency error the wizards died at the first line of real work with `dbx: line 2002: require_gum: command not found`, whether or not `gum` was installed. Since `dbx storage add` is the only interactive way to configure a storage backend, that blocked S3/R2 setup on a fresh install. `require_gum` is now defined next to `require_jq` / `require_docker` in `lib/core.sh` and fails with an install hint. A test asserts every `require_*` helper called from `dbx` or `lib/` is actually defined, so the next one can't go missing.

- **`dbx vault set` can store a storage backend's S3 secret (#210).** The secret's home is the vault — `lib/storage.sh` reads it back from `s3-secret-key-<name>` (or `s3-secret-key` for the legacy single `storage` block) on every upload — but `vault set` validated every name against `.hosts[]` and answered `Host 's3-secret-key-r2' not found in config`, while `vault get` read the same key happily. With `storage add` broken (#209) that left no supported writer at all; the only ways in were hand-editing `config.json` with `secret_key_cmd` or inventing a dummy host. `vault set` now recognises a storage-secret key, checks the backend it names is configured, and prompts for the secret key (showing the endpoint and bucket it belongs to). Unknown names are still refused, with the configured backends listed: `Storage 'typo' not found in config. Configured backends: r2. Run: dbx storage add`.

- **`dbx clean --older-than` no longer dies on macOS when GNU coreutils are on PATH (#142).** The age check picked its `stat` syntax from `is_macos` — i.e. from the OS — but the flavour of `stat` actually on PATH is independent of it. On a Mac with nix or Homebrew `gnubin` ahead of `/usr/bin`, dbx took the BSD branch and hit `stat: invalid option -- '%'`, leaving the arithmetic with an empty operand and aborting the prune with `arithmetic syntax error: operand expected`. It now probes both dialects (`stat -f%m` then `stat -c%Y`), the same idiom already used for the size lookup a few lines below. A backup whose mtime cannot be read is now kept with a warning rather than treated as infinitely old — the previous shape would have deleted it.
...
- **`mc` is declared as the dependency it has always been (#211).** `lib/storage.sh` shells out to the MinIO client for every S3 operation, but `mc` was only ever installed in the Docker image — `install.sh` didn't check for it and nothing checked at runtime, so on a source install every storage operation failed without ever naming the cause. `install.sh` now reports it when neither `mc` nor the `aws` CLI is on PATH, listing where to get each; it does not install either, and the report is kept separate from the hard-dependency block because storage is opt-in, either client will do, and `apt install mc` is Midnight Commander, not this. The runtime message now names both clients and links their install docs, and it survives to the terminal instead of being swallowed by `dbx storage test`.

- **`dbx storage test` says why it failed (#212).** Every failure printed `[INFO] storage test: upload` and exited 1: `mc alias set` ran under `>/dev/null 2>&1`, and the round-trip ran each step under the same redirection, so "mc not installed", "secret is empty", "wrong credentials" and "bucket unreachable" were byte-identical. Each step now keeps the client's stderr and drops only its chatter (`2>&1 >/dev/null`, the same idiom as #201), and prints it under the failing step. Failures are told apart: a missing config field is named (`missing: endpoint access_key`), an `s3.secret_key_cmd` that produces nothing is blamed on the command with its own stderr shown rather than on the config, and mc's own text ("Unable to initialize new alias…", "Bucket b does not exist") reaches the user. The S3 secret is redacted from anything surfaced — it is on `mc alias set`'s argv, so mc can quote it back — and a test asserts a sentinel secret never appears.

- **A failed upload, download or delete no longer reports success (#212).** `storage_upload`, `storage_download` and `storage_delete` each ended in an optional-sidecar block whose exit status became the function's, so `mc cp` failing returned 0: a backup with S3 offload reported success while the object was never written, and `dbx storage test` misattributed the failure to the *list* step. The primary transfer's status is now the function's; the `.meta.json` / `.globals.sql` sidecars stay best-effort.
